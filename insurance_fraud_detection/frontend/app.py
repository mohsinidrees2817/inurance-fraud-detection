...
def load_sample_data():
    try:
        ...
        st.warning(
            "⚠️ Sample data file not found in frontend container. Using default thresholds for display purposes."
        )
        return None
...
def main():
    st.markdown(
        '<div class="main-header">🚗 Insurance Fraud Detection System</div>',
        unsafe_allow_html=True,
    )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["🔍 Fraud Detection", "📊 Model Information", "📈 Analytics Dashboard"]
    )

    with tab1:
        st.sidebar.markdown(
            '<div class="sidebar-header">📝 Enter Claim Details</div>',
            unsafe_allow_html=True,
        )

        with st.sidebar.expander("👤 Customer Information", expanded=True):
            months_as_customer = st.number_input(
                "Months as Customer", min_value=1, max_value=500, value=100
            )
        ...
        with st.sidebar.expander("📋 Policy Information", expanded=True):
            ...
            policy_state = st.selectbox(
                "Policy State", ["OH", "IN", "IL", "WV", "VA", "NY", "SC", "NC"]
            )
        ...
        with st.sidebar.expander("🚨 Incident Details", expanded=True):
            ...
            witnesses = st.number_input(
                "Number of Witnesses", min_value=0, max_value=10, value=2
            )
        ...
        with st.sidebar.expander("💰 Claim Details", expanded=True):
            ...
            with col1:
                ...
                injury_claim = st.number_input(
                    "Injury Claim ($)",
                    min_value=0.0,
                    max_value=total_claim_amount,
                    value=min(2000.0, total_claim_amount // 4),
                )
            with col2:
                # FIX: Ensure min_value and max_value are floats for float 'value'
                property_claim = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=total_claim_amount,
                    value=min(5000.0, total_claim_amount // 4),
                )
            remaining_amount = max(
                0.0, total_claim_amount - injury_claim - property_claim
            )
            # FIX: Ensure min_value and max_value are floats for float 'value'
            vehicle_claim = st.number_input(
                ...
                min_value=0.0,
                max_value=total_claim_amount,
                value=min(remaining_amount, total_claim_amount),
            )
            total_individual = injury_claim + property_claim + vehicle_claim
            if total_individual != total_claim_amount:
                st.warning(
                    f"⚠️ Individual claims ({total_individual:,.0f}) don't sum to total claim amount ({total_claim_amount:,.0f})"
                )
        ...
        with st.sidebar.expander("🚗 Vehicle Information", expanded=True):
            ...
            auto_year = st.number_input(
                "Auto Year", min_value=1990, max_value=datetime.now().year, value=2010
            )
                ...
                with st.spinner("Analyzing claim data..."):
                    ...
                    try:
                        ...
                        response = requests.post(
                            f"{FASTAPI_URL}/predict", json=input_data
                        )
                        ...
                        with col_prob1:
                            gauge_fig = create_gauge_chart(
                                fraud_probability, "Fraud Risk Score"
                            )
                        ...
                        st.error(
                            f"❌ Error communicating with backend: {str(e)}. Make sure backend is running and accessible at {FASTAPI_URL}"
                        )
                    except Exception as e:
        ...
        with col2:
            ...
            confidence_placeholder = (
                st.empty()
            )  # This placeholder is not actively updated in the current code
    ...
    with tab2:
        ...
        st.info(
            "This section is a placeholder. You can expand it to fetch model details from the backend or display static information about your model's performance metrics (e.g., accuracy, precision, recall)."
        )
        if sample_df is not None:
    ...
    with tab3:
        ...
        if sample_df is not None:
            ...
            with col1:
                fraud_by_state = (
                    sample_df.groupby("policy_state")["fraud_reported"]
